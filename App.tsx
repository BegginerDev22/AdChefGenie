...
const CUISINE_FILTERS = ["Indian", "Italian", "Chinese", "Mexican", "Asian", "American", "Mediterranean", "Thai", "Japanese", "French"];
      ...
      <main className="max-w-6xl mx-auto px-4 pt-10">
        ...
        {viewMode !== 'planner' && (
          <>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border border-orange-100 dark:border-gray-700 rounded-xl shadow-xl max-h-60 overflow-y-auto z-50 custom-scrollbar">
                    <ul>
                      {suggestions.map((suggestion, index) => (
                        <li 
                          key={index}
                          onMouseDown={() => handleAddIngredient(suggestion)}
            ...
            {(sourceHasRecipes || viewMode === 'saved') && (
        ...
        {viewMode === 'planner' ? (
          ...
          <div className="max-w-4xl mx-auto space-y-6 animate-[fadeIn_0.3s_ease-out]">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                ...
                {viewMode === 'saved' 
                    ? `Cookbook (${displayRecipes.length})` 
                    : hasGenerated 
                    ...
                    <div className="h-48 overflow-hidden relative">
                        ...
                        {recipe.cuisine && (
                            <div className="absolute top-2 left-2 z-10">
                                <span className={`${cuisineColor} backdrop-blur-md px-2 py-1 rounded-md text-[10px] font-bold text-white shadow-md flex items-center gap-1 border border-white/20`}>
                                    🌍 {recipe.cuisine}
                                </span>
                            </div>
                        )}
